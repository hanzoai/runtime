...
ASYNC_CHANGES=$(git diff --cached --name-only | grep "libs/sdk-python/src/runtime/_async/.*\.py$" || true)
SYNC_CHANGES=$(git diff --cached --name-only | grep "libs/sdk-python/src/runtime/_sync/.*\.py$" || true)
...
if [ -n "$ASYNC_CHANGES" ] || [ -n "$SYNC_CHANGES" ]; then
    ...
    isort libs/sdk-python/src/runtime/_sync
    black libs/sdk-python/src/runtime/_sync --config pyproject.toml
    
    # Check if there are any new changes after running the sync generator
    NEW_CHANGES=$(git diff --name-only libs/sdk-python/src/runtime/_sync)
    ...
    if [ -n "$NEW_CHANGES" ]; then
        ...
        echo "  git add libs/sdk-python/src/runtime/_sync/"
        echo ""

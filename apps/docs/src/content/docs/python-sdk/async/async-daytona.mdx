...
Main class for interacting with the Runtime API.

This class provides asynchronous methods to create, manage, and interact with Runtime Sandboxes.
...
```python
async with AsyncRuntime() as runtime:  # Uses RUNTIME_API_KEY, RUNTIME_API_URL
...
try:
    runtime = AsyncRuntime(config)
    sandbox = await runtime.create()
finally:
    await runtime.close()
```

#### AsyncRuntime.\_\_init\_\_

```python
def __init__(config: Optional[RuntimeConfig] = None)
```

Initializes Runtime instance with optional configuration.

If no config is provided, reads from environment variables:
- `RUNTIME_API_KEY`: Required API key for authentication
- `RUNTIME_API_URL`: Required api URL
- `RUNTIME_TARGET`: Optional target environment (defaults to 'us')

**Arguments**:

- `config` _Optional[RuntimeConfig]_ - Object containing api_key, api_url, and target.
  

**Raises**:

- `RuntimeError` - If API key is not provided either through config or environment variables
...
runtime2 = AsyncRuntime(config)
await runtime2.close()
```

#### AsyncRuntime.\_\_aenter\_\_
...
This method should be called when you're done using the AsyncRuntime instance
...
- `params` _Optional[CreateSandboxFromSnapshotParams]_ - Parameters for Sandbox creation. If not provided,
  defaults to default Runtime snapshot and Python language.
...
- `RuntimeError` - If timeout, auto_stop_interval or auto_archive_interval is negative;
...
sandbox = await runtime.create(params, timeout=40)
```
...
Creates Sandboxes from specified image available on some registry or declarative Runtime Image.
You can specify various parameters, including resources, language, image, environment variables,
and volumes. Runtime creates snapshot from provided image and uses it to create Sandbox.
...
```python
sandbox = await runtime.create(CreateSandboxFromImageParams(image="debian:12.9"))
```
...
- `RuntimeError` - If sandbox fails to delete or times out
...
sandbox = await runtime.create()
# ... use sandbox ...
await runtime.delete(sandbox)  # Clean up when done
```
...
- `RuntimeError` - If sandbox_id is not provided.
  

**Example**:

```python
sandbox = await runtime.get("my-sandbox-id")
print(sandbox.status)
```

#### AsyncRuntime.find\_one
...
- `RuntimeError` - If no Sandbox is found.
  

**Example**:

```python
sandbox = await runtime.find_one(labels={"my-label": "my-value"})
...
```python
sandboxes = await runtime.list(labels={"my-label": "my-value"})
...
- `RuntimeError` - If timeout is negative; If Sandbox fails to start or times out
...
- `RuntimeError` - If timeout is negative; If Sandbox fails to stop or times out
...
Programming languages supported by Runtime
...
Configuration options for initializing the Runtime client.

**Attributes**:

- `api_key` _Optional[str]_ - API key for authentication with the Runtime API. If not set, it must be provided
  via the environment variable `RUNTIME_API_KEY`, or a JWT token must be provided instead.
- `jwt_token` _Optional[str]_ - JWT token for authentication with the Runtime API. If not set, it must be provided
  via the environment variable `RUNTIME_JWT_TOKEN`, or an API key must be provided instead.
- `organization_id` _Optional[str]_ - Organization ID used for JWT-based authentication. Required if a JWT token
  is provided, and must be set either here or in the environment variable `RUNTIME_ORGANIZATION_ID`.
- `api_url` _Optional[str]_ - URL of the Runtime API. Defaults to `'https://app.hanzo.ai/api'` if not set
  here or in the environment variable `RUNTIME_API_URL`.
...
- `target` _Optional[str]_ - Target runner location for the Sandbox. Defaults to `'us'` if not set here
  or in the environment variable `RUNTIME_TARGET`.
...
```
```python
config = RuntimeConfig(jwt_token="your-jwt-token", organization_id="your-organization-id")
```

...
Provides methods for creating, managing, and interacting with Runtime Sandboxes.
Can be initialized either with explicit configuration or using environment variables.

**Properties**:

- `snapshot` _SnapshotService_ - Service for managing Runtime Snapshots
- `volume` _VolumeService_ - Service for managing Runtime Volumes
...
// Uses RUNTIME_API_KEY, RUNTIME_API_URL, RUNTIME_TARGET
...
```ts
new Runtime(config?: RuntimeConfig): Runtime
```
...
- `config?` _RuntimeConfig_ - Configuration options
...
- `RuntimeError` - When API key is missing
...
const sandbox = await runtime.create(params, { timeout: 100 });
```
...
Creates Sandboxes from specified image available on some registry or declarative Runtime Image. You can specify various parameters,
including resources, language, image, environment variables, and volumes. Runtime creates snapshot from
...
```ts
const sandbox = await runtime.create({ image: 'debian:12.9' }, { timeout: 90, onSnapshotCreateLogs: console.log });
```
...
const sandbox = await runtime.create(params, { timeout: 100, onSnapshotCreateLogs: console.log });
```
...
```ts
const sandbox = await runtime.get('my-sandbox-id');
await runtime.delete(sandbox);
```
...
```ts
const sandbox = await runtime.findOne({ labels: { 'my-label': 'my-value' } });
...
```ts
const sandboxes = await runtime.list({ 'my-label': 'my-value' });
...
await runtime.start(sandbox, 60);
```
...
- `apiKey?` _string_ - API key for authentication with the Runtime API
- `apiUrl?` _string_ - URL of the Runtime API. Defaults to 'https://app.hanzo.ai/api'
    if not set here and not set in environment variable RUNTIME_API_URL.
- `jwtToken?` _string_ - JWT token for authentication with the Runtime API. If not set, it must be provided

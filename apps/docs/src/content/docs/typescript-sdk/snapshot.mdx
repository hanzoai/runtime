...
Service for managing Runtime Snapshots. Can be used to list, get, create and delete Snapshots.
...
await runtime.snapshot.create({ name: 'my-snapshot', image: image }, { onLogs: console.log });
```
...
const snapshot = await runtime.snapshot.get("snapshot-name");
...
```ts
const runtime = new Runtime();
const snapshots = await runtime.snapshot.list();
...
Represents a Runtime Snapshot which is a pre-configured sandbox.

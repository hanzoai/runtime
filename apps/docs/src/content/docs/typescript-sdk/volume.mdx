...
Service for managing Runtime Volumes.
...
const volume = await runtime.volume.create("my-data-volume");
...
```ts
const runtime = new Runtime();
const volume = await runtime.volume.get("volume-name");
...
```ts
const runtime = new Runtime();
const volume = await runtime.volume.get("volume-name", true);
...
```ts
const runtime = new Runtime();
const volumes = await runtime.volume.list();
...
Represents a Runtime Volume which is a shared storage volume for Sandboxes.

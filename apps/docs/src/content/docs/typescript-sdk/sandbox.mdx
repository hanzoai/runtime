...
- `runnerDomain?` _string_ - Domain name of the Sandbox runner
    ...
    ```
- `snapshot?` _string_ - Runtime snapshot used to create the Sandbox
...
- `RuntimeError` - If interval is not a non-negative integer
...
- `RuntimeError` - If Sandbox fails to start or times out

**Example:**

```ts
const sandbox = await runtime.getCurrentSandbox('my-sandbox');
...
- `RuntimeError` - If the sandbox ends up in an error state or fails to start within the timeout period.
...
- `RuntimeError` - If the sandbox fails to stop within the timeout period.
## SandboxCodeToolbox

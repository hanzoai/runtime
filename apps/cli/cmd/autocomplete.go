...
var AutoCompleteCmd = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		if shell == "bash" {
			...
			fmt.Println("On how to install bash-completion, please refer to the following link: https://www.hanzo.ai/docs/tools/cli/#runtime-autocomplete")
		}
...
func SetupAutocompletionForShell(rootCmd *cobra.Command, shell string) (string, error) {
	...
	switch shell {
	case "bash":
		filePath = filepath.Join(homeDir, ".runtime.completion_script.bash")
		profilePath = filepath.Join(homeDir, ".bashrc")
	case "zsh":
		filePath = filepath.Join(homeDir, ".runtime.completion_script.zsh")
		profilePath = filepath.Join(homeDir, ".zshrc")
	case "fish":
		filePath = filepath.Join(homeDir, ".config", "fish", "runtime.completion_script.fish")
		profilePath = filepath.Join(homeDir, ".config", "fish", "config.fish")
	case "powershell":
		filePath = filepath.Join(homeDir, "runtime.completion_script.ps1")
